  ...
  return (
    <section className="flex h-full min-h-0 w-full flex-col overflow-hidden bg-white">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-4 pb-6 scrollbar-thin-custom">
        ...
        <div className="flex flex-col gap-4 self-stretch sm:flex-row sm:flex-wrap">
...
function ManifestCard({
  ...
  return (
    <div className="flex flex-1 flex-col gap-3 rounded-xl border border-card-outline bg-white p-4">
        ...
        {skills.length > 0 ? (
          skills.map((skill) => {
            ...
            return (
              <div
                key={skill.slug}
                className="rounded-lg border border-tag-border overflow-hidden"
              >
                <button
                  onClick={() => setOpenSlug(isOpen ? null : skill.slug)}
                  className="flex w-full items-center justify-between px-3 py-2 text-left"
                >
                  <span className="font-sans text-[11px] text-folder-fg">
                    {skill.name}
                  </span>
                  <ChevronDown
                    className={`h-3 w-3 text-folder-fg transition-transform ${isOpen ? "rotate-180" : ""}`}
                  />
                </button>
                {isOpen && skill.content && (
                  <div className="border-t border-tag-border px-3 py-3">
