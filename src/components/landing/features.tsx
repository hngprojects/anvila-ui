...
import { CARDS } from "@/data/landing/features";
import Link from "next/link";

const STATS = [
  { label: "Setup time", value: "< 3 min" },
  { label: "Generation accuracy", value: "99%+" },
];

export function Features() {
  return (
    <section className="w-full bg-zinc-100">
      <div className="mx-auto flex w-full max-w-[960px] flex-col items-center px-6 py-14 md:py-20">
        {/* Pill */}
        ...
        {/* Heading */}
        <h2 className="mb-4 max-w-[640px] text-center text-[26px] font-semibold leading-tight tracking-tight text-zinc-900 md:text-[40px]">
          One setup. Clean files.
          <br className="hidden md:block" /> Reusable agents.
        </h2>

        {/* Subhead */}
        <p className="mb-8 max-w-[520px] text-center text-sm leading-relaxed text-zinc-500 md:text-[15px]">
        ...
        <div className="mb-10 flex flex-wrap items-center justify-center gap-3 md:mb-12">
          {STATS.map((stat) => (
            <div
              key={stat.label}
              className="flex items-center gap-2 rounded-full border-[0.5px] border-zinc-300 bg-white px-4 py-2 shadow-[0_1px_2px_rgba(0,0,0,0.04)]"
            >
        ...
        {/* Cards */}
        <div className="mb-10 grid w-full grid-cols-1 gap-4 sm:grid-cols-2 md:mb-12 md:grid-cols-3">
          {CARDS.map((card) => {
            const isDark = card.variant === "dark";
            return (
              <div
                key={card.title}
                className={`group relative flex flex-col gap-4 overflow-hidden rounded-[20px] p-6 transition-shadow duration-200 ${
                  isDark
                    ? "border border-white/15 bg-teal-brand shadow-[0_8px_24px_-8px_rgba(12,93,86,0.45)]"
                    : "border border-zinc-200 bg-white hover:shadow-[0_8px_24px_-8px_rgba(0,0,0,0.08)]"
                } ${isDark ? "" : "sm:last:col-span-2 md:last:col-span-1"}`}
              >
                {/* Subtle corner glow */}
                <div
                  aria-hidden
                  className={`pointer-events-none absolute -right-10 -top-10 h-28 w-28 rounded-full transition-transform duration-300 group-hover:scale-110 ${
