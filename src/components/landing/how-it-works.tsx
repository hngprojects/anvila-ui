...
function ClipboardPencilIcon({ stroke }: { stroke: string }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="32"
...
function AtomIcon({ stroke }: { stroke: string }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="32"
...
const STEPS: Step[] = [
  {
    Icon: GridIcon,
    desktopStroke: "#DB2777",
    bgClasses: "bg-teal-brand md:bg-[#FCE7F3]",
  ...
  {
    Icon: AtomIcon,
    desktopStroke: "#9333EA",
    bgClasses: "bg-teal-brand md:bg-[#F3E8FF]",
...
export function HowItWorks() {
  return (
    <section className="relative w-full bg-[#FBFBFB]">
      <div className="mx-auto flex w-full max-w-[1439px] flex-col items-center gap-6 px-6 py-12 md:px-20 md:py-16">
        {/* Pill */}
        <div className="inline-flex items-center gap-2 rounded-full border-[0.5px] border-zinc-400 bg-transparent px-4 py-2">
          <span
            aria-hidden="true"
        ...
        <h2 className="m-0 text-center text-3xl font-medium leading-[38px] text-logo md:text-5xl md:leading-[60px]">
          How Anvila Works
        </h2>

    
        <p className="m-0 mx-auto max-w-[600px] text-center text-sm font-normal leading-5 text-copy-muted md:text-base md:leading-6">
          Effortlessly create organized GitHub-Ready agent setup files you can
          reuse, publish, and adapt across projects in less time.
        </p>

    
        <div className="mt-6 flex w-full flex-col items-center gap-10 self-stretch md:mt-12 md:grid md:grid-cols-3 md:gap-8 md:pb-10">
          {STEPS.map((step) => {
            const { Icon } = step;
            return (
              <div
                key={step.title}
                className="flex w-full flex-col items-center gap-4 self-stretch md:gap-6"
              >
    
                <div
                  className={`flex h-[38px] w-[38px] items-center justify-center rounded-full p-1.5 md:h-[72px] md:w-[72px] md:p-5 ${step.bgClasses}`}
                >
                  ...
                  <span className="hidden md:block">
                    <Icon stroke={step.desktopStroke} />
                  </span>
                </div>

                <h3 className="m-0 text-center text-xl font-medium leading-[30px] text-zinc-900">
                  {step.title}
                </h3>

                <p className="m-0 max-w-[231px] text-center text-sm font-normal leading-5 text-copy-muted md:text-base md:leading-6">
                  {step.description}
                </p>
              </div>
            );
