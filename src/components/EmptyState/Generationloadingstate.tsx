...
function computeStepStates(steps: typeof GENERATION_STEPS, elapsed: number): StepState[] {
  let cursor = 0;
  return steps.map((s) => {
    const start = cursor;
...
function Spinner() {
  return (
    <div className="w-16 h-16 mx-auto mb-5">
      <svg viewBox="0 0 64 64" className="w-full h-full animate-spin"
        style={{ animationDuration: "1.2s", animationTimingFunction: "linear" }}>
        <circle cx="32" cy="32" r="26" className="stroke-green-100" strokeWidth="5" fill="none" />
        <circle cx="32" cy="32" r="26" stroke="#0C5D56" strokeWidth="5" fill="none"
          strokeLinecap="round" strokeDasharray="60 200" strokeDashoffset="0" />
      </svg>
    </div>
  );
}
...
function StepIcon({ state }: { state: StepState }) {
  ...
  if (state === "active") return (
    <div className="flex-shrink-0 w-6 h-6 rounded-full bg-green-50 border-2 border-[#0C5D56] flex items-center justify-center">
      <svg className="w-3 h-3 animate-spin" style={{ animationDuration: "1.2s" }}
...
export default function GenerationLoadingState({
  ...
  const totalDuration = useMemo(() => steps.reduce((sum, s) => sum + s.durationSeconds, 0), [steps]);
  ...
  useEffect(() => {
    tickRef.current = setInterval(() => {
      setElapsed((prev) => {
        if (prev >= totalDuration) { clearInterval(tickRef.current!); return totalDuration; }
        return prev + 0.1;
      });
    }, 100);
    ...
    return () => { clearInterval(tickRef.current!); clearInterval(msgRef.current!); };
  }, [totalDuration]);

  const stepStates = useMemo(() => computeStepStates(steps, elapsed), [steps, elapsed]);
  const pct = totalDuration > 0 ? Math.min(Math.round((elapsed / totalDuration) * 100), 99) : 0;
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center px-4">
      <div className="w-full max-w-md bg-white border border-gray-100 rounded-2xl shadow-sm px-10 py-12 text-center">
        ...
        <h1 className="text-xl font-semibold text-gray-900 mb-2">Building your agent…</h1>
        <p className="text-sm text-gray-500 leading-relaxed mb-7">This usually takes 15–30 seconds. Hang tight.</p>
        ...
        <div className="mb-6 text-left">
          ...
          <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-[#0C5D56] rounded-full transition-all duration-300" style={{ width: `${pct}%` }} />
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 mb-6 text-left min-h-[52px]">
          ...
          <p className="text-xs text-gray-600 leading-relaxed">{PROGRESS_MESSAGES[msgIndex]}</p>
        </div>
          ...
          {steps.map((step, i) => {
            ...
            return (
              <div key={step.id} className={[
                ...
                state === "active" ? "bg-white border-brand-primary"    :
                                     "bg-gray-50 border-gray-100",
              ].join(" ")}>
                <StepIcon state={state} />
                <span className={["flex-1 text-sm font-medium",
                  state === "pending" ? "text-gray-400" : "text-gray-800"].join(" ")}>
                  {step.name}
                </span>
                <span className={["text-xs",
                  state === "done"   ? "text-brand-primary font-medium" :
                  state === "active" ? "text-brand-primary font-medium" :
                                       "text-gray-400"].join(" ")}>
        ...
        <div className="mt-6 flex items-center justify-center gap-1.5">
          ...
          <span className="text-xs text-gray-400">Your session is secure — generation runs server-side</span>
        </div>
      </div>
    </div>
  );
