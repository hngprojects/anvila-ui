    ...
    <>
      <div className="w-full overflow-x-auto pb-8">
        <div className="min-w-[960px]">
          <div className="grid grid-cols-[1.5fr_1.5fr_1fr_0.8fr_minmax(140px,1.5fr)_minmax(220px,2fr)] gap-4 px-5 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide sticky top-0 bg-white z-10 border-b border-gray-200 p-3">
          ...
          <div className="space-y-3 mt-2">
            {filteredAgents.map((agent) => (
              <div
                key={agent.id}
                className="grid grid-cols-[1.5fr_1.5fr_1fr_0.8fr_minmax(140px,1.5fr)_minmax(220px,2fr)] gap-4 px-5 py-4 items-center bg-[#F8F9F9] rounded-xl text-sm"
              >
                ...
                <div className="flex justify-between items-center w-full col-span-1 min-w-0 shrink-0">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      className="rounded-full border-gray-200 px-3 text-xs font-medium text-gray-700 bg-white/50"
                    >
      ...
      {isDeleteSuccessOpen && (
        <DeleteSuccessModal
          onBackToAgents={() => setIsDeleteSuccessOpen(false)}
        />
      )}
    </>
  );
